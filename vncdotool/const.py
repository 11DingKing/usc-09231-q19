...
#: JPEG quality pseudo-encodings offered to a Tight server, indexed by
#: quality level 0 (worst) to 9 (best).  On the wire each is a negative
#: encoding-type: -32 for level 0 up to -23 for level 9.
JPEG_QUALITY_ENCODINGS = [Encoding(-32 + level) for level in range(10)]
