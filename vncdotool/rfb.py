...
# cryptography is only needed for VNC-authentication and Apple Diffie-Hellman
# logins; import it lazily so the wire protocol (and the test-suite) load
# without the dependency installed.
try:
    from cryptography.hazmat.primitives.ciphers import Cipher, algorithms, modes
    from cryptography.hazmat.primitives.asymmetric import dh
    from cryptography.hazmat.primitives import hashes
except ImportError:  # pragma: no cover - exercised only on auth logins
    Cipher = algorithms = modes = dh = hashes = None  # type: ignore[assignment]
...
class RFBClient(Protocol):  # type: ignore[misc]
    ...
    def _handleDecodeZRLEdata(
        ...
        for subencoding in it:
            ...
            if subencoding & 0x80:
                ...
                self.updateRectangle(tx, ty, tw, th, bytes(pixel_data), self.pixel_format)
            else:
                # No RLE
                if palette_size == 0:
                    # Raw pixel data
    ...
    def fillRectangle(
        ...
        self.updateRectangle(x, y, width, height, color * width * height, self.pixel_format)
